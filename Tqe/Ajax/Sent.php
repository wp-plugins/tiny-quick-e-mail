...
class Tqe_Ajax_Sent implements Tqe_Ajax_Interface
{
    ...
    private function _del()
    {
        ...
        $this->_result->noticeMessage(sprintf('E-mail%s moved to trash.', $plural));
    ...
    private function _delHelper($id)
    {
        ...
        if ($result < 1)
         ...
         if ($wpdb->update($wpdb->qe_mails, array(
            'type'    => 3,
            'deleted' => date_i18n('Y-m-d H:i:s')
         ), array(
            'id'     => $id,
            'userid' => Tqe_Registry::get('userInfo')->ID,
            'type'   => 1
        ), array('%d', '%s'), array('%d', '%d', '%d')) === false)
            throw new Exception('Could not move e-mail to trash.');
    ...
    private function _view()
    {
        ...
        $sql = 'SELECT `id`, `to`, `useCc`, `cc`, `useBcc`, `bcc`, `subject`, `message`, UNIX_TIMESTAMP(`updated`) '
        . 'AS `date` '
        . 'FROM `' . $wpdb->qe_mails . '` '
        . 'WHERE `userid` = %d '
        . 'AND `type` = 1 '
    ...
    private function _list()
    {
        ...
        if ($wpdb->query($wpdb->prepare($sql, Tqe_Registry::get('userInfo')->ID)) === false)
            throw new Exception('Could retrieve total count of sent e-mails.');
        ...
        if ($wpdb->last_result[0]->count < 1)
            ...
            $sql = 'SELECT `message`, `subject`, UNIX_TIMESTAMP(`updated`) AS `date`, `id` '
            . 'FROM `' . $wpdb->qe_mails . '` '
            . 'WHERE `userid` = %d '
            . 'AND `type` = 1 ';
            ...
            if ($wpdb->query($wpdb->prepare($sql, (($currPage * $limit) - $limit), $limit)) === false)
                throw new Exception('Could retrieve sent e-mails.');
            ...
            $this->_toTemplate->displaying = sprintf(
                '<span class="displaying-num">%s</span>', sprintf(
                    __( 'Displaying sent e-mails %s&#8211;%s of %s' ),
    ...
    private function _initTemplate()
    {
        ...
        $template->assignFile(sprintf('sent/%s.phtml', retGet('action')));
