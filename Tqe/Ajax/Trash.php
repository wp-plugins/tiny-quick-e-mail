...
class Tqe_Ajax_Trash implements Tqe_Ajax_Interface
{
    ...
    private function _restore()
    {
        if (retPost('post')) {
            ...
            foreach (retPost('ids') as $id)
                $this->_restoreHelper($id);
        }
        else {
             if (!retGet('id'))
                throw new Exception('You did not provide e-mail ID.');

             $this->_restoreHelper(retGet('id'));
        }


        $plural = retPost('post') === false ? '' : (count(retPost('ids')) > 1 ? 's' : '');

        $this->_result->noticeMessage(sprintf('E-mail%s restored.', $plural));
    ...
    private function _restoreHelper($id)
    {
        ...
        if ($result < 1)
         ...
         if ($wpdb->update($wpdb->qe_mails, array(
            'type'    => 1,
            ...
            'type'   => 3
        ), array('%d', '%s'), array('%d', '%d', '%d')) === false)
            throw new Exception('Could not restore e-mail.');
    }
    ...
    private function _list()
    {
        ...
        if ($wpdb->query($wpdb->prepare($sql, Tqe_Registry::get('userInfo')->ID)) === false)
            throw new Exception('Could retrieve total count of e-mails in trash.');
        ...
        if ($wpdb->last_result[0]->count < 1)
            ...
            if (retGet('order')) {
                switch (retGet('order')) {
                    case 'date' :
                        $sql .= sprintf('ORDER BY UNIX_TIMESTAMP(`%s`) ', 'deleted');
                        break;
                    case 'subject' :
                    default :
            ...
            if ($wpdb->query($wpdb->prepare($sql, (($currPage * $limit) - $limit), $limit)) === false)
                throw new Exception('Could retrieve e-mails in trash.');
            ...
            $this->_toTemplate->displaying = sprintf(
                '<span class="displaying-num">%s</span>', sprintf(
                    __( 'Displaying e-mails in trash %s&#8211;%s of %s' ),
    ...
    private function _initTemplate()
    {
        ...
        $template->assignFile(sprintf('trash/%s.phtml', retGet('action')));
